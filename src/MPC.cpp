...
class FG_eval {
 public:
  ...
  const double weight_cte = 5;
  const double weight_epsi = 3;
  const double weight_v = 1;
  const double weight_delta = 20000;
  const double weight_a = 50;
  const double weight_ddelta = 20000;
  const double weight_da = 50;
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (t = 0; t < N; t++) {
      ...
      fg[0] += weight_v * CppAD::pow(vars[v_start + t] - v_ref, 2);
    }
